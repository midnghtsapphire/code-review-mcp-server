...
export async function scanAccessibility(args: any) {
...
function getWCAGLevel(ruleId: string | null): 'A' | 'AA' | 'AAA' {
  const levelAARules = [
  ...
  if (ruleId && levelAAARules.includes(ruleId)) return 'AAA';
  if (ruleId && levelAARules.includes(ruleId)) return 'AA';
  return 'A';
}

function getAccessibilitySuggestion(ruleId: string | null): string {
